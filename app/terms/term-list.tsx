    ...
    <main className="md:w-10/12 w-11/12 my-10 mx-auto">
      <div className="space-y-8">
        {termsconditions.map((item, index) => (
          <div key={index}>
            <h2 className="font-bold  lg:text-2xl md:text-xl text-md uppercase border-b-4 pb-1 mb-8 w-fit border-primary2/60  ">
             {index + 1}.{item.title}
            </h2>
            {item.points.map((point, pointIndex) => (
              <div key={pointIndex} className="my-2">
                <p className="font-medium md:text-base text-sm text-zinc-800 flex gap-4">
                  <span>• </span>
